...
void AngleClass2::compute(int eflag, int vflag)
{
  ...
  for (n = 0; n < nanglelist; n++) {
    ...
    if (eflag) eangle = k2[type]*dtheta2 + k3[type]*dtheta3 + k4[type]*dtheta4 + k5[type]*dtheta5 + k6[type]*dtheta6;
...
void AngleClass2::coeff(int narg, char **arg)
{
  ...
  if (strcmp(arg[1],"bb") == 0) {
    ...
    //
    double k5_one = force->numeric(arg[5]);
    double k6_one = force->numeric(arg[6]);
...
double AngleClass2::single(int type, int i1, int i2, int i3)
{
  ...
  double energy = k2[type]*dtheta2 + k3[type]*dtheta3 + k4[type]*dtheta4 + k5[type]*dtheta5 + k6[type]*dtheta6;
